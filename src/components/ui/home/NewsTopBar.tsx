...
export default function NewsTopBar({
  ...
  const handlePrint = () => {
    ...
    if (!win) return;
    win.document.write(`
      <html><head><title>ই-পেপার - পৃষ্ঠা ${activePage.pageNumber}</title>
      <style>body{margin:0;display:flex;justify-content:center;}img{max-width:100%;height:auto;}</style>
      </head><body><img src="${activePage.image}" onload="window.print();window.close()"/></body></html>
    `);
    win.document.close();
  };

  const shareUrl = typeof window !== "undefined" ? window.location.href : "";
  const shareText = activePage?.epaperTitle || "ই-পেপার";
  ...
  const socialLinks = [
    ...
    },
    {
      name: "LinkedIn",
      icon: <LinkedInIcon />,
      color: "bg-[#0A66C2] hover:bg-[#095ba9]",
    ...
    <div className="bg-white border-b border-gray-200 shadow-sm sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 py-2 flex items-center gap-2 flex-wrap">
        ...
        {activePage?.epaperDate && (
          <div className="flex items-center gap-1 border border-gray-200 rounded px-2 py-1 text-sm bg-gray-50">
            <span>📅</span>
        ...
        <button
          ...
          className="p-1.5 hover:bg-gray-100 rounded border border-gray-200 disabled:opacity-40 disabled:cursor-not-allowed transition-all"
        >
          <ChevronLeft className="w-4 h-4 text-gray-600" />
        </button>

        {/* ✅ FIX: Page buttons — index ব্যবহার করে serial number 1,2,3... সঠিকভাবে দেখাচ্ছে */}
        <div className="flex items-center gap-1 flex-wrap">
          {pages.map((page, index) => (
            <button
              key={`page-btn-${index}`}
              onClick={() => onPageChange(index + 1)}
              className={`min-w-[28px] h-7 px-1.5 text-xs rounded border font-medium transition-all ${
                activeIndex === index
          ...
          {(
            ...
            <button
              ...
              className={`px-3 py-1.5 text-xs rounded border font-medium transition-all ${
                viewMode === mode ? activeClass : inactiveClass
              }`}
            >
              {label}
            </button>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <div className="relative" ref={shareRef}>
            <button
              ...
              className="px-3 py-1.5 text-xs rounded border border-gray-200 font-medium bg-white text-gray-700 hover:bg-gray-50 flex items-center gap-1.5 transition-all"
            >
              <Share2 className="w-3.5 h-3.5" /> শেয়ার
            </button>

            {shareOpen && (
              <div className="absolute right-0 top-full mt-2 bg-white border border-gray-200 rounded-xl shadow-xl p-3 z-50 w-52">
                ...
                <div className="grid grid-cols-4 gap-2 mb-3">
                  {socialLinks.map((s: any) => (
                    <a
                      ...
                      title={s.name}
                      className={`${s.color} text-white rounded-lg p-2 flex items-center justify-center transition-all`}
                    >
                      {s.icon}
                    </a>
                  ))}
                ...
                <button
                  onClick={handleCopy}
                  className="w-full flex items-center gap-2 px-3 py-2 text-xs rounded-lg border border-gray-200 hover:bg-gray-50 transition-all mb-1"
                >
                ...
                <button
                  ...
                  className="w-full flex items-center gap-2 px-3 py-2 text-xs rounded-lg border border-gray-200 hover:bg-gray-50 transition-all"
                >
