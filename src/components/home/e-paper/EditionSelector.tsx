...
import { EditionSelectorProps } from "@/types/epaper";
  ...
  return (
    <div className="relative flex-shrink-0" ref={editionRef}>
      <button onClick={() => setEditionOpen(!editionOpen)} className={pill}>
        <Newspaper className="w-5 h-3.5 text-gray-500 text-sm font-semibold" />
        <span>{displayEdition}</span>
        <ChevronDown
          className={`w-5 font-semibold text-sm  h-3 text-gray-400 transition-transform duration-200 ${editionOpen ? "rotate-180" : ""}`}
        />
      </button>

      {editionOpen && finalEditions.length > 0 && (
        <div className="absolute left-0 top-full  bg-white rounded shadow-2xl border border-gray-100 py-1.5 z-[9999] min-w-[100px]">
